  ...
  .action(async (blockchain) => {
    try {
      ...
      if (buildResult.exitCode !== 0) {
        throw new Error(`chr build failed with exit code ${buildResult.exitCode}`);
      }
      ...
      const xmlFiles = fs.readdirSync('build').filter(file => file.endsWith(`${blockchain}.xml`));
      if (xmlFiles.length !== 1) {
        throw new Error(`Expected exactly one XML file for blockchain ${blockchain}, found ${xmlFiles.length}`);
      }
